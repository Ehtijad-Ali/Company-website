...
function FaqItem({ q, a, i, inView }) {
  ...
  return (
    <motion.div
      ...
      transition={{delay:.7+i*.06, duration: DUR.reveal, ease: E }}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ height:0, opacity:0 }} transition={{ duration:.28, ease: E }}
    ...
    <section id="contact" ref={ref} className="section relative" style={{ background:'var(--bg-surface)', overflow:'hidden', borderTop:'1px solid var(--border)' }}>
      ...
      <div className="container relative" style={{ zIndex: 1 }}>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 mb-20">
          ...
          {/* Left */}
          <motion.div initial={{opacity: 0, y: RISE }} animate={inView?{opacity:1,y: 0}:{}} transition={{duration: DUR.reveal, ease: E }}
            className="space-y-7">
            {/* Terminal */}
            <div className="rounded-xl overflow-hidden" style={{ background:'var(--bg-card)', border:'1px solid var(--border)' }}>
              ...
              <div className="p-5 space-y-2 min-h-[140px]">
                {(contact.terminal ?? []).map(({ text, color, delay }, i) => (
                  /* Terminal output appears; it does not rise. */
                  <motion.p key={i} initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
                    transition={{ delay, duration: DUR.ui }}
            ...
            <div className="space-y-3">
              {contact.channels.map(({ icon, label, value }, i) => {
                ...
                return (
                <motion.div key={label} initial={{opacity:0, y: RISE }} animate={inView?{opacity:1,y:0}:{}} transition={{duration: DUR.reveal, delay:.4+i*.09, ease: E }}
            ...
            <motion.a href={contact.whatsapp?.href} target="_blank" rel="noopener noreferrer"
              initial={{opacity:0, y: RISE }} animate={inView?{opacity:1,y:0}:{}} transition={{duration: DUR.reveal, delay:.7, ease: E }}
            ...
            <div className="flex gap-2">
              {(contact.socials ?? []).map(({ icon, href, label }, i) => {
                const Icon = iconFor(icon)
                return (
                <motion.a key={label} href={href} title={label}
                  initial={{ opacity: 0, y: RISE }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: .85 + i * STAGGER, duration: DUR.reveal, ease: E }}
                  whileHover={{ y:-3 }}
        ...
        {/* FAQ */}
        <motion.div initial={{opacity:0, y: RISE }} animate={inView?{opacity:1,y:0}:{}} transition={{delay:.4, duration: DUR.reveal, ease: E }}
